...
class Trainer():
    ...
    def get_batch(self):
        ...
        if self.args.dset.name == "maestro_allyears":
            ...
            audio = t_utils.resample_batch(audio, fs, self.args.exp.sample_rate, self.args.exp.audio_len)
        else:
        ...
        # --- Inpainting mask/gap generation ---
        B, T = audio.shape
        gap_min = getattr(self.args, 'inpainting', {}).get('gap_min_size', 2048)
        gap_max = getattr(self.args, 'inpainting', {}).get('gap_max_size', 8192)
        gap_num = getattr(self.args, 'inpainting', {}).get('gap_num', 1)
        gap_position = getattr(self.args, 'inpainting', {}).get('gap_position', 'random')
        ...
        for b in range(B):
            ...
            for _ in range(gap_num):
                gap_size = np.random.randint(gap_min, gap_max+1)
            ...
            gap_info.append(gaps)

        context = audio * mask
        gap = audio * (1 - mask)

        # Add noise only to the gap region
        noise = torch.randn_like(audio)
        audio_noisy = context + noise * (1 - mask)

        return audio_noisy, context, mask, gap_info
    def train_step(self):
        # Train step
        ...
        for round_idx in range(self.args.exp.num_accumulation_rounds):
            # Get batch with inpainting mask and context
            audio_noisy, context, mask, gap_info = self.get_batch()

            # Pass audio_noisy, context, and mask to the loss function
            error, sigma = self.diff_params.loss_fn(self.network, audio_noisy, mask=mask, context=context)
            loss = error.mean()
